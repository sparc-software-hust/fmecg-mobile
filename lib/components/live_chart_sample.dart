...
class _LiveLineChartState extends State<LiveLineChart> {
  _LiveLineChartState() {
    _timer = Timer.periodic(const Duration(milliseconds: 200), _updateDataSource);
  }
  ...
  Widget build(BuildContext context) {
    return Material(child: _buildLiveLineChart());
  }
    ...
    return SfCartesianChart(
      ...
      series: <LineSeries<_ChartSampleData, int>>[
        LineSeries<_ChartSampleData, int>(
          ...
          animationDuration: 0,
          onRendererCreated: (ChartSeriesController<_ChartSampleData, int> controller) {
  ...
  void _updateDataSource(Timer timer) {
    _chartData!.add(_ChartSampleData(_count, _generateRandomInteger(10, 100)));
    if (_chartData!.length == 20) {
      ...
      _chartSeriesController?.updateDataSource(addedDataIndexes: <int>[_chartData!.length - 1]);
    }
    _count = _count + 1;
  }
